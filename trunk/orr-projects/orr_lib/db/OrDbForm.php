...
class OrDbForm extends OrDojoForm {
  ...
  function reload($sql)
  {
		...
		if($this->OP_[cur_rec]->get()<1)$this->OP_[cur_rec]->set(1);
		if($this->OP_[total_rec]->get()<1)$this->OP_[total_rec]->set(1);
		/*แสดงรายการเดิม*/
  ...
  function get_sec_user()
  {
		...
		$this->db->get_query($sql);
		if($this->db->is_error())die('ไม่สามารถตรวจสอบเจ้าของข้อมูลได้ กรุณาตรวจสอบกับผู้ดูแลระบบ'.$this->db->show_error());
		$this->db->get_record();
  ...
  function add_record()
  {
		...
		$this->db->get_query($sql);
		//die("<b>debug</b> ".__FILE__." | ".__LINE__." | sql =".$sql."<br>");
		if($this->db->is_error())die('ไม่สามารถเพิ่มข้อมูลได้ กรุณาตรวจสอบกับผู้ดูแลระบบ!'.$this->db->show_error());
		$EV_[EV_controls] = $this->val_controls[db_field];
  ...
  function del_record()
  {
		...
		$this->db->close_conn();
		if(!$g_my_sec->can_del())die("ไม่สิทธิ์การลบข้อมูล<br>");
		$sql="DELETE FROM ".$this->db_table;
		$sql.= $this->get_where_key().";";
		//die("test" . $this->get_where_key());
		$this->db->get_query($sql);
		if($this->db->is_error())die('ไม่สามารถลบข้อมูลได้ กรุณาตรวจสอบกับผู้ดูแลระบบ'.$this->db->show_error());
  ...
  function save_record()
  {
		...
		if(!$g_my_sec->can_save())die("ไม่มีสิทธิ์การบันทึกข้อมูล");
		/*จัดการค่าที่ได้รับจาก field ใน form*/
		...
		//die("<b>debug</b> ".__FILE__." | ".__LINE__." | sql =".$sql."<br>");
		$this->db->get_query($sql);
		if($this->db->is_error())die('ไม่สามารถบันทึกข้อมูล '.$this->db->show_error());
