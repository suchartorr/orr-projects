...
class my extends my_page {
	function __construct()
	{
		 ...
		 */
		$sql = "SELECT *  FROM `my_datafield`";//<-กำหนดคำสั่ง SQL
		
		$my_form = new OrDbFrmList('my_form' , $this->get_my_db() );
                $my_form->OP_[edit_page_url]->set('my_datafield.php');//กำหนด URL ของหน้าแก้ไขข้อมูล
                $my_form->OP_[edit_field_link]->set('field_id');//กำหนด ชื่อ Field ที่ต้องการให้เป็น Link หนาแก้ไขข้อมูล
                $my_form->OP_[edit_key_field]->set('field_id'); //กำหนด ชื่อ Field ที่เป็นคีย์แก้ไข
		...
		$my_form->set_controls(new OrLabel('field_id'));
