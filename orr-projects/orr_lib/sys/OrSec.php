...
class OrSec extends OrObj {
  ...
  function get_authority()
  {
		...
		if($this->OP_[sys_id]->get()!="")/*ตรวจสอบรหัสโปรแกรม*/
		{
			...
			if($this->OP_[user]->get()!="" and $db_sec->record[sys_id]==$this->OP_[sys_id]->get() )
			{
				...
				if($this->OP_[user]->get()==$this->OP_[sec_user]->get())$fun_val=$db_sec->record[aut_user];
				/*ตรวจสอบสิทธิ์ในระดับกลุ่ม*/
				if($this->chk_aut_group($this->OP_[sec_user]->get()))if($db_sec->record[aut_group]>$fun_val)$fun_val=$db_sec->record[aut_group];
					 /*ตรวจสอบสิทธิ์ในระดับผู้ใช้งานอื่นๆ*/
				if($db_sec->record[aut_any]>$fun_val)$fun_val=$db_sec->record[aut_any];
				}
			}
		}
		//$db_sec->close_conn();
  ...
  function activity($description)
  {
		...
		$db_sec->get_query($sql);
		if($db_sec->is_error())$db_sec->show_error();
		return null;
  }
