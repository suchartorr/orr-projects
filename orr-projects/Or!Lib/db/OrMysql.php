 ...
 */
 class OrMysql extends OrDb {
  ...
  function get_host_conn($database = null,$pconnect = false){
  	global $my_cfg_db;
  	if(!is_null($database))$this->OP_[database]->set($database);
  	if($pconnect){
		...
		if(!mysql_select_db($this->OP_[database]->get(),$conn))die("ö¡ Database".mysql_error());
		
		$charset = "SET NAMES '" . $my_cfg_db[charset] . "'";
		if($my_cfg_db[charset]!= '')mysql_query($charset, $conn) or die(__FILE__." | ".__LINE__." |get_host_conn | ".mysql_error());
   ...
   */
	function get_query($sql){
		...
		if(!$result)$this->set_error('sql='.$sql.' ['.mysql_error().']<br>');
   ...
   */
	...
	}// end of member function close_conn
