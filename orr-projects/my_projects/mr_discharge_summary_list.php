...
class my extends my_page {
	function __construct()
	{
		 ...
		 */
		$sql = "SELECT `mr_discharge_summary`.`id`, `mr_discharge_summary`.`discharge_date`, `mr_discharge_summary`.`iopd`, `mr_discharge_summary`.`hn`, CONCAT( `patient`.`prefix`, `patient`.`fname`, '  ', `patient`.`lname` ) AS `name`, `mr_discharge_summary`.`an`, `patient`.`sex`, `mr_discharge_summary`.`age_year`, `mr_discharge_summary`.`age_month`, `mr_discharge_summary`.`principal_diag1`, `mr_diag`.`name_e`, `mr_discharge_summary`.`external_cause`, `mr_discharge_summary`.`consultation1`, `mr_discharge_summary`.`death1a`, `mr_discharge_summary`.`signature`, CONCAT( `doctor`.`prefix`, `doctor`.`fname`, '  ', `doctor`.`lname` ) AS `doctor_name` FROM `theptarin_utf8`.`mr_discharge_summary` AS `mr_discharge_summary`, `theptarin_utf8`.`patient` AS `patient`, `theptarin_utf8`.`doctor` AS `doctor`, `theptarin_utf8`.`mr_diag` AS `mr_diag` WHERE `mr_discharge_summary`.`hn` = `patient`.`hn` AND `mr_discharge_summary`.`signature` = `doctor`.`doctor_code` AND `mr_discharge_summary`.`principal_diag1` = `mr_diag`.`code` AND `mr_discharge_summary`.`iopd` = 'I'";//<-กำหนดคำสั่ง SQL
		
		$my_form = new OrDbFrmList('my_form' , $this->get_my_db() );
                $my_form->OP_[edit_page_url]->set('mr_ipd_search_list.php');//กำหนด URL ของหน้าแก้ไขข้อมูล
		...
		$my_form->set_controls(new OrLabel('signature'),'Signature');
