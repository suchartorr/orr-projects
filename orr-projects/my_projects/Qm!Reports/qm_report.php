...
class my extends my_page {
    ...
    function __construct() {
        ...
        $my_form->OP_[list_page_url]->set('qm_reports_list.php');
        ...
        $my_form->set_controls(new OrDojoSelectbox('report_id'));
        $my_form->controls[report_id]->OP_[option]->set($this->get_report_id());

        $my_form->set_controls(new OrTextCalendar('report_date'));

        $my_form->set_controls(new OrDojoTextbox('report_value'));
    ...
    function get_report_id() {
        ...
        $sql = "SELECT * FROM `qm_report_name` ORDER BY `name` ASC"; //(กำหนด SQL ตามเงื่อนไขที่ต้องการ)
        $db->get_query($sql);
        while ($db->get_record()) {
